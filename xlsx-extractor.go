...
func RowsWithTitles(book io.Reader, sheet string, titles []string, dateFieldsList ...[]string) ([]string, <-chan []string, error) {
	...
	go func() {
		for rows.Next() {
			arr, _ := rows.Columns()
			if cols := adjustCol(arr, colCount, dateCols); len(cols) == 0 {
				continue
			} else {
...
func adjustCol(cols []string, colCount int, dateCols map[int]bool) []string {
	...
	return adjustDate(aCols, dateCols)
}
...
func adjustDate(cols []string, dateCols map[int]bool) []string {
	if len(dateCols) == 0 {
		return cols
	}
	for i, _ := range cols {
